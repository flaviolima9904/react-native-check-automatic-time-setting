# react-native-check-automatic-time-setting

A React Native library to check if the device's automatic time setting is enabled on Android. **(NOT AVAILABLE for IOS)**

## Installation

1. **Install the library:**
   ```sh
   npm install react-native-check-automatic-time-setting
   ```
   or
   ```sh
   yarn add react-native-check-automatic-time-setting
   ```
...
2. **Link the native modules:**
   ...
   ```sh
   react-native link react-native-check-automatic-time-setting
   ```
